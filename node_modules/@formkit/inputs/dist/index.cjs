...
function createLibraryPlugin(...libraries) {
  const library = libraries.reduce(
    (merged, lib) => utils.extend(merged, lib),
    {}
  );
  const plugin = () => {
  };
  ...
  return function(prop, next) {
    if (prop.prop === "options" && Array.isArray(prop.value)) {
      prop.value = prop.value.map((option2) => {
        if (!option2.attrs?.id) {
          return utils.extend(option2, {
            attrs: {
              id: `${node.props.id}-option-${utils.slugify(String(option2.value))}`
            }
          });
        }
        return option2;
...
var loading = /* @__PURE__ */ core.createMessage({
  key: "loading",
  value: true,
  visible: false
});
  ...
  if (node.ledger.value("blocking")) {
    ...
    if (typeof node.props.onSubmit === "function") {
      const retVal = node.props.onSubmit(
        node.hook.submit.dispatch(utils.clone(node.value)),
        node
      );
...
function select(node) {
  node.on("created", () => {
    const isMultiple = utils.undefine(node.props.attrs?.multiple);
        ...
        const hasMatchingValue = node.props.options.some(
          (option2) => {
            if (option2.attrs && "data-is-placeholder" in option2.attrs)
              return false;
            const optionValue2 = "__original" in option2 ? option2.__original : option2.value;
            return utils.eq(value, optionValue2);
          }
        );
  ...
  node.hook.input((value, next) => {
    if (!node.props.placeholder && value === void 0 && Array.isArray(node.props?.options) && node.props.options.length && !utils.undefine(node.props?.attrs?.multiple)) {
...
function isSlotCondition(node) {
  if (core.isConditional(node) && node.if && node.if.startsWith("$slots.") && typeof node.then === "string" && node.then.startsWith("$slots.") && "else" in node) {
    return true;
  }
  return false;
}
...
function eachSection(schema, callback, stopOnCallbackReturn = false, schemaParent) {
  ...
  } else if ((core.isComponent(schema) || core.isDOM(schema)) && schema.children) {
    return eachSection(
      schema.children,
      callback,
      stopOnCallbackReturn
    );
  } else if (core.isConditional(schema)) {
...
function isSection(section) {
  if (core.isConditional(section) && typeof section.then === "string" && section.else && typeof section.else !== "string" && !Array.isArray(section.else) && !core.isConditional(section.else) && section.else.meta?.section) {
    return true;
  }
  return false;
}
...
function useSchema(inputSection, sectionsSchema = {}) {
  ...
  return (propSectionsSchema = {}) => schema(utils.extend(sectionsSchema, propSectionsSchema));
}
// @__NO_SIDE_EFFECTS__
function $attrs(attrs, section) {
  const extendable = (extensions) => {
    ...
    if (!utils.isObject(attributes))
      return node;
    if (/* @__PURE__ */ isSlotCondition(node) && core.isDOM(node.else)) {
