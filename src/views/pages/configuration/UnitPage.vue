...
var { addUnit, fetchAllUnit, isLoading } = useUnitsStore()

const unit_store = useUnitsStore()
const { units, message, hasError, status } = storeToRefs(unit_store)
...
const filterData = computed(() => {
    if (searchInput.value.trim() === "") {
        ...
        const result = units.value.filter((unit) => {
            return unit.unit.toLowerCase().includes(searchInput.value.toLowerCase())
        });
        return result
    }
});
...
<template>
    <div class="ml-2">
        <header>
            <BaseHead @show-form="displayShow" @handleInput="input" @show-list="displayList" :Header="pageName" />
        </header>
        ...
        <div class="flex w-full justify-end  h-screen text-gray-700">
            <div class="lg:flex-row md:flex sm:flex-col  w-full ">



                <div class="flex  w-full flex-grow border-r border-gray-300">
                ...
                option }}
                        ...
                        <Table :headers="tableHeader">
                            <tr v-for="(unit, index) in paginated" :key="index.unit"
                                class="bg-none h-4 hover:bg-slate-200">
                                <td>{{ index + 1 }}</td>
                                <td>{{ unit.unit }}</td>
            ...
            <div class="flex flex-col md:flex-row sm:flex-row flex-shrink-0 md:w-1/4 w-full py-1 pl-4">
                <div class="flex w-full py-1 border-b border-gray-300 ">
                    <div class="flex flex-col flex-grow mx-2">

                        <FormKit v-model="unit" type="text" label="Unit Name" name="unit" validation="required" />

                        <button :disabled="isValid" @click="createUnit()" :class="{ 'disabled-button': isValid }"
